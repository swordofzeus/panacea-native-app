...
class User(Base):
    ...
    # Relationships
    patient_medications = relationship("PatientMedication", back_populates="user")
    responses = relationship("Response", back_populates="user")

    def __repr__(self):
        return f"<User(username={self.username}, email={self.email})>"

# Association table for the many-to-many relationship between MedicationCatalog and PatientMetric
medication_metric_association = Table(
    "medication_metric",
    Base.metadata,
    Column("medication_id", String, ForeignKey("medication_catalog.id"), primary_key=True),
    Column("metric_id", String, ForeignKey("patient_metrics.id"), primary_key=True),
)
...
class MedicationCatalog(Base):
    ...
    metrics = relationship(
        "PatientMetric",
        secondary=medication_metric_association,
        back_populates="medications",
    )
    questions = relationship("Question", back_populates="medication")

    def __repr__(self):
        return f"<MedicationCatalog(name={self.name})>"
...
class PatientMetric(Base):
    ...
    def __repr__(self):
        return f"<PatientMetric(name={self.name})>"
...
class PatientMedication(Base):
    ...
    medication_id = Column(String, ForeignKey("medication_catalog.id"), nullable=False)
    dosage = Column(String, nullable=True)
    schedule = Column(String, nullable=True)
    notes = Column(Text, nullable=True)

    # Relationships
    user = relationship("User", back_populates="patient_medications")
    medication = relationship("MedicationCatalog")

    def __repr__(self):
        return f"<PatientMedication(user_id={self.user_id}, medication_id={self.medication_id})>"
...
class Question(Base):
    ...
    input_type = Column(String, nullable=False, default="choice")  # e.g., "choice" or "text"
    medication_id = Column(String, ForeignKey("medication_catalog.id"))
    metric_id = Column(String, ForeignKey("patient_metrics.id"))

    # Relationships
    medication = relationship("MedicationCatalog", back_populates="questions")
    metric = relationship("PatientMetric", back_populates="questions")

    def __repr__(self):
        return f"<Question(text={self.question_text}, input_type={self.input_type})>"
...
class Response(Base):
    ...
    user_id = Column(String, ForeignKey("users.id"), nullable=False)
    medication_id = Column(String, ForeignKey("medication_catalog.id"), nullable=False)
    metric_id = Column(String, ForeignKey("patient_metrics.id"), nullable=False)
    ...
    medication = relationship("MedicationCatalog")
    metric = relationship("PatientMetric")
    question = relationship("Question")

    def __repr__(self):
        return f"<Response(user_id={self.user_id}, answer={self.answer})>"
