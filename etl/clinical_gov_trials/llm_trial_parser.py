...
class LLMStudyParser:
    ...
    def transform_interventions(self, data, session, study_id):
        ...
        """
        intervention_data = data.get("protocolSection", {}).get("armsInterventionsModule", {}).get("interventions", [])
        ...
        for fragment in intervention_data:
            ...
            if response:
                ...
                response["id"] = response.get("id") if response.get("id") else response.get("name")
                try:
    ...
    def transform_conditions(self, data, session, study_id):
        ...
        """
        conditions_data = data.get("protocolSection", {}).get("conditionsModule", {}).get("conditions", [])
    ...
    def transform_keywords(self, data, session, study_id):
        ...
        """
        keywords_data = data.get("protocolSection", {}).get("conditionsModule", {}).get("keywords", [])
    ...
    def transform_phases(self, data, session, study_id):
        ...
        """
        phases_data = data.get("protocolSection", {}).get("designModule", {}).get("phases", [])
    ...
    def transform_participant_denoms(self, data, session, study_id):
        ...
        """
        denom_data = data.get("resultsSection").get("baselineCharacteristicsModule", {}).get("denoms", [])
    ...
    def transform_arms(self, data, session, study_id):
        ...
        """
        arms_data = data.get("protocolSection", {}).get("armsInterventionsModule", {}).get("armGroups", [])
    ...
    def transform_adverse_event_groups(self, data, session, study_id):
        ...
        """
        event_groups = data.get("resultsSection", {}).get("adverseEventsModule", {}).get("eventGroups", [])
    ...
    def transform_adverse_events(self, data, session, study_id):
        ...
        for event_type in adverse_event_types:
            events = data.get("resultsSection", {}).get("adverseEventsModule", {}).get(event_type, [])
    ...
    def transform_participant_statistics(self, data, session, study_id):
        ...
        """
        measure_data = data.get("resultsSection", {}).get("baselineCharacteristicsModule", {}).get("measures", [])
    ...
    def transform_outcomes(self, data, session, study_id):
        ...
        """
        outcomes_data = data.get("protocolSection", {}).get("outcomesModule", {}).get("primaryOutcomes", [])
        ...
        for fragment in outcomes_data:
            ...
            if response:
                response["study_id"] = study_id
                response["id"] = response.get("id") if response.get("id") else response.get("measure")
    ...
    def transform_contacts(self, data, session, study_id):
        ...
        """
        contacts_data = data.get("protocolSection", {}).get("contactsLocationsModule", {}).get("overallOfficials", [])
    ...
    def transform_arms(self, data, session, study_id):
        ...
        for fragment in arms_data:
            ...
            if response:
                response["study_id"] = study_id
                response["id"] = response.get("id") if response.get("id") else response.get("label")
                try:
    ...
    def transform_participant_groups(self, data, session, study_id):
        ...
        """
        group_data = data.get("resultsSection").get("baselineCharacteristicsModule", {}).get("groups", [])
            ...
            for class_data in measure.get("classes", []):
                for category in class_data.get("categories", []):
                    for measurement in category.get("measurements", []):
                        ...
                        prompt = generate_schema_prompt(ParticipantDemographic, fragment)
                        response = call_llm(prompt)
                        if response:
                            response["study_id"] = study_id
                            response["group_id"] = response.get("group_id")
    ...
    def run(self):
        ...
        try:
            ...
            studies = base_session.query(Study).filter(Study.processed_at == None).all()
        except Exception as e:
